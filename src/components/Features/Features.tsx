        ...
        <div className="accessories d-flex flex-wrap gap-3">
          {
            featuresStyles.map((feature, index) => {
              return <FeatureBtn text={feature.type} active={ index === activeFeature ? true: false } clickHandler={() => featureClickHandler(index)}/>
            })
          }
        </div>
        ...
        <div className="style d-flex flex-wrap gap-3">
          {
            featuresStyles[activeFeature].styles.map((featureStyle, index) => {
              return <FeatureBtn text={featureStyle} active={ featuresStyles[activeFeature].activeStyle === featureStyle ? true: false } clickHandler={() => featureStyleClickHandler(index)} />
            })
          }
        </div>
    </div>
